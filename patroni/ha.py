...
class _MemberStatus(NamedTuple):
    ...
    def from_api_response(cls, member: Member, json: Dict[str, Any]) -> "_MemberStatus":
        ...
        in_recovery = not bool(wal.get("location")) or json.get("role") in (
            "master",
            "primary",
        )
        ...
        lsn = int(
            in_recovery
            and max(wal.get("received_location", 0), wal.get("replayed_location", 0))
        )
        return cls(
            member,
            True,
            ...
            lsn,
            json.get("tags", {}),
            json.get("watchdog_failed", False),
        )
        ...
        with self._lock:
            if self._last_update + self._dcs.ttl > time.time() and self._name:
                return Leader(
                    "",
                    "",
                    RemoteMember.from_name_and_data(
                        self._name,
                        {
...
class Ha(object):
    def __init__(self, patroni: Patroni):
        ...
        self._async_executor = AsyncExecutor(
            self.state_handler.cancellable, self.wakeup
        )
    ...
    def acquire_lock(self) -> bool:
        try:
            ...
            logger.exception(
                "Unexpected exception raised from attempt_to_acquire_leader, please report it as a BUG"
            )
            ret = False
    ...
    def update_lock(self, write_leader_optime: bool = False) -> bool:
        ...
        try:
            ret = self.dcs.update_leader(
                self.cluster.leader, last_lsn, slots, self._failsafe_config()
            )
        except DCSError:
            raise
        except Exception:
            logger.exception(
                "Unexpected exception raised from update_leader, please report it as a BUG"
            )
            ret = False
        self.set_is_leader(ret)
        if ret:
        ...
        if self.state_handler.citus_handler.is_worker():
            ...
            if coordinator and coordinator.leader and coordinator.leader.conn_url:
                try:
                    ...
                    logger.warning(
                        "Request to Citus coordinator leader %s %s failed: %r",
                        coordinator.leader.name,
                        coordinator.leader.member.api_url,
                        e,
                    )
    ...
    def touch_member(self) -> bool:
        with self._member_state_lock:
            ...
            if self._async_executor.scheduled_action in (None, "promote") and data[
                "state"
            ] in ["running", "restarting", "starting"]:
                try:
                    (
                        timeline,
                        wal_position,
                    ...
                    if not timeline:
                        ...
                        if self.state_handler.role == "standby_leader":
                            timeline = (
                                pg_control_timeline
                                or self.state_handler.pg_control_timeline()
                            )
                        else:
                            timeline = (
                                self.state_handler.replica_cached_timeline(
                                    self._leader_timeline
                                )
                                or 0
                            )
                    if timeline:
            ...
            if ret:
                new_state = (
                    data["state"],
                    {"master": "primary"}.get(data["role"], data["role"]),
                )
                if self._last_state != new_state and new_state == (
                    "running",
                    "primary",
                ):
    ...
    def clone(
        self,
        clone_member: Union[Leader, Member, None] = None,
        msg: str = "(without leader)",
    ) -> Optional[bool]:
    ...
    def bootstrap(self) -> str:
        ...
        if (
            self.cluster.is_unlocked()
            and self.cluster.initialize is None
            and not self.patroni.nofailover
            and "bootstrap" in self.patroni.config
        ):
            if self.dcs.initialize(create_new=True):  # race for initialization
                ...
                if self.is_standby_cluster():
                    ret = self._async_executor.try_run_async(
                        "bootstrap_standby_leader", self.bootstrap_standby_leader
                    )
        ...
        if not self.cluster.is_unlocked() and clone_member:
            ...
            ret = self._async_executor.try_run_async(
                "bootstrap {0}".format(msg), self.clone, args=(clone_member, msg)
            )
        ...
        create_replica_methods = (
            self.global_config.get_standby_cluster_config().get(
                "create_replica_methods", []
            )
            if self.is_standby_cluster()
            else None
        ...
        can_bootstrap = (
            self.state_handler.can_create_replica_without_replication_connection(
                create_replica_methods
            )
        )
        ...
        if can_bootstrap and not concurrent_bootstrap:
            ...
            return (
                self._async_executor.try_run_async(msg, self.clone)
                or "trying to " + msg
            )
    ...
    def _handle_crash_recovery(self) -> Optional[str]:
        if self._crash_recovery_started == 0 and (
            ...
            return (
                self._async_executor.try_run_async(
                    msg, self._rewind.ensure_clean_shutdown
                )
                or msg
            )
    ...
    def _handle_rewind_or_reinitialize(self) -> Optional[str]:
        leader = (
            self.get_remote_member()
            if self.is_standby_cluster()
            else self.cluster.leader
        )
        if (
            not self._rewind.rewind_or_reinitialize_needed_and_possible(leader)
            or not leader
        ):
            return None
        ...
        if self._rewind.can_rewind:
            ...
            if (
                not self.state_handler.is_running()
                and self.state_handler.controldata().get("Database cluster state")
                == "in archive recovery"
            ):
            ...
            return (
                self._async_executor.try_run_async(
                    msg, self._rewind.execute, args=(leader,)
                )
                or msg
            )
        ...
        if (
            ...
            return (
                self._async_executor.try_run_async(
                    msg, self._do_reinitialize, args=(self.cluster,)
                )
                or msg
            )
    ...
    def recover(self) -> str:
        ...
        if (
            timeout
            and data.get("Database cluster state")
            in ("in production", "in crash recovery", "shutting down", "shut down")
            ...
            if (
                self._async_executor.try_run_async(
                    ...
                    args=(timeout, self._async_executor.critical_task),
                )
                is None
            ):
        ...
        if data.get("Database cluster state") in (
            "in production",
            "shutting down",
            "in crash recovery",
        ):
        ...
        if (
            self._async_executor.try_run_async(
                ...
                self.state_handler.follow,
                args=(node_to_follow, role, timeout),
            )
            is None
        ):
            self.recovering = True
    ...
    def _get_node_to_follow(self, cluster: Cluster) -> Union[Leader, Member, None]:
        ...
        if self.is_standby_cluster() and (
            self.cluster.is_unlocked() or self.has_lock(False)
        ):
            node_to_follow = self.get_remote_member()
        elif (
            self.patroni.replicatefrom
            and self.patroni.replicatefrom != self.state_handler.name
        ):
            node_to_follow = cluster.get_member(self.patroni.replicatefrom)
        else:
            node_to_follow = (
                cluster.leader if cluster.leader and cluster.leader.name else None
            )
        ...
        if node_to_follow and not isinstance(node_to_follow, RemoteMember):
            ...
            for (
                ...
                params
            ):  # It is highly unlikely to happen, but we want to protect from the case
                node_to_follow.data.pop(
                    param, None
                )  # when above-mentioned params came from outside.
            if self.is_standby_cluster():
                standby_config = self.global_config.get_standby_cluster_config()
                node_to_follow.data.update(
                    {p: standby_config[p] for p in params if standby_config.get(p)}
                )

        return node_to_follow

    def follow(
        self, demote_reason: str, follow_reason: str, refresh: bool = True
    ) -> str:
        if refresh:
            self.load_cluster_from_dcs()
        ...
        if self.is_paused():
            if (
                not (
                    ...
                    and self._rewind.can_rewind_or_reinitialize_allowed
                )
        ...
        if (
            ...
            and self._leader_timeline
            and self.state_handler.get_history(self._leader_timeline + 1)
        ):
            self._rewind.trigger_check_diverged_lsn()
        ...
        role = (
            "standby_leader"
            if isinstance(node_to_follow, RemoteMember) and self.has_lock(False)
            else "replica"
        )
        ...
        if (
            ...
            (
                change_required,
                restart_required,
            ) = self.state_handler.config.check_recovery_conf(node_to_follow)
            if change_required:
                if restart_required:
                    self._async_executor.try_run_async(
                        "changing primary_conninfo and restarting",
                        self.state_handler.follow,
                        args=(node_to_follow, role),
                    )
                else:
    ...
    def process_sync_replication(self) -> None:
        ...
        if self.is_synchronous_mode():
            ...
            picked, allow_promote = self.state_handler.sync_handler.current_state(
                self.cluster
            )
            ...
            if picked != current:
                ...
                if sync_common != current:
                    logger.info(
                        "Updating synchronous privilege temporarily from %s to %s",
                        list(current),
                        list(sync_common),
                    )
                    sync = self.dcs.write_sync_state(
                        self.state_handler.name, sync_common, version=sync.version
                    )
                    if not sync:
                        return logger.info(
                ...
                if (
                    picked
                    and picked != CaseInsensitiveSet("*")
                    and allow_promote != picked
                ):
                    # Wait for PostgreSQL to enable synchronous mode and see if we can immediately set sync_standby
                    time.sleep(2)
                    _, allow_promote = self.state_handler.sync_handler.current_state(
                        self.cluster
                    )
                if allow_promote and allow_promote != sync_common:
                    if not self.dcs.write_sync_state(
                        self.state_handler.name, allow_promote, version=sync.version
                    ):
                        return logger.info(
                            "Synchronous replication key updated by someone else"
                        )
                    logger.info(
                        "Synchronous standby status assigned to %s", list(allow_promote)
                    )
        else:
            if not self.cluster.sync.is_empty and self.dcs.delete_sync_state(
    ...
    def is_sync_standby(self, cluster: Cluster) -> bool:
        ...
        return (
            bool(cluster.leader)
            and cluster.sync.leader_matches(cluster.leader.name)
            ...
            if self.touch_member():
                ...
                for _ in polling_loop(timeout=self.dcs.loop_wait * 2, interval=2):
                    try:
                        ...
                        break
                    logger.info(
                        "Waiting for primary to release us from synchronous standby"
                    )
            else:
                logger.warning(
    ...
    def update_cluster_history(self) -> None:
        ...
        if primary_timeline == 1:
            ...
            not cluster_history
            or cluster_history[-1][0] != primary_timeline - 1
            or len(cluster_history[-1]) != 5
        ):
            cluster_history = {line[0]: line for line in cluster_history}
            history: List[List[Any]] = list(
                map(list, self.state_handler.get_history(primary_timeline))
            )
            ...
            for line in history:
                ...
                if (
                    len(line) == 3
                    and len(cluster_history_line) >= 4
                    and cluster_history_line[1] == line[1]
                ):
    ...
    def enforce_primary_role(self, message: str, promote_message: str) -> str:
        ...
        if not self.is_paused():
            ...
            with self._async_response:
                if self._async_response.result is False:
                    logger.warning(
                        "Releasing the leader key voluntarily because the pre-promote script failed"
                    )
        ...
        if self.state_handler.is_leader():
            ...
            if self.is_synchronous_mode():
                ...
                if not self.dcs.write_sync_state(
                    self.state_handler.name, None, version=self.cluster.sync.version
                ):
                ...
                self.state_handler.sync_handler.set_synchronous_standby_names(
                    CaseInsensitiveSet("*")
                    if self.global_config.is_synchronous_mode_strict
                    else CaseInsensitiveSet()
                )
    ...
    def fetch_node_status(self, member: Member) -> _MemberStatus:
        ...
        try:
            ...
            logger.info(
                "Got response from %s %s: %s", member.name, member.api_url, data
            )
            return _MemberStatus.from_api_response(member, json.loads(data))
        except Exception as e:
            logger.warning(
                "Request failed to %s: GET %s (%s)", member.name, member.api_url, e
            )
        return _MemberStatus.unknown(member)

    def fetch_node_status_and_priority(
        self, member: Member
    ) -> Tuple[_MemberStatus, int]:
        """This function perform http get request on member.api_url and fetches its status
        and it's failover_priority.
        ...
        try:
            ...
            return (
                ...
                member.tags.get("failover_priority", 1),
            )
        except Exception as e:
            logger.warning(
                "Request failed to %s: GET %s (%s)", member.name, member.api_url, e
    ...
    def fetch_nodes_statuses(self, members: List[Member]) -> List[_MemberStatus]:
        ...
        results = pool.map(
            self.fetch_node_status, members
        )  # Run API calls on members in parallel
        pool.close()
        pool.join()
        return results

    def fetch_nodes_statuses_and_priorities(
        self, members: List[Member]
    ) -> List[Tuple[_MemberStatus, int]]:
        """Gets the statuses and priorities of a list of members in parallel"""
        pool = ThreadPool(len(members))
        results = pool.map(
            self.fetch_node_status_and_priority, members
        )  # Run API calls on members in parallel
        pool.close()
        pool.join()
        return results
    ...
    def update_failsafe(self, data: Dict[str, Any]) -> Optional[str]:
        if self.state_handler.state == "running" and self.state_handler.role in (
            "master",
            "primary",
        ):
    ...
    def call_failsafe_member(self, data: Dict[str, Any], member: Member) -> bool:
        try:
            response = self.patroni.request(
                member, "post", "failsafe", data, timeout=2, retries=1
            )
            ...
            logger.warning(
                "Request failed to %s: POST %s (%s)", member.name, member.api_url, e
            )
        return False
    ...
    def check_failsafe_topology(self) -> bool:
        ...
        members = [
            RemoteMember.from_name_and_data(name, {"api_url": url})
    ...
    def _is_healthiest_node(
        self, members: Collection[Member], check_replication_lag: bool = True
    ) -> bool:
        ...
        if not self.is_standby_cluster() and self.check_timeline():
            ...
            if my_timeline < cluster_timeline:
                logger.info(
                    "My timeline %s is behind last known cluster timeline %s",
                    my_timeline,
                    cluster_timeline,
                )
                return False
        ...
        members = [
            m
            for m in members
            if m.name != self.state_handler.name and not m.nofailover and m.api_url
        ]
            ...
            for st, priority in self.fetch_nodes_statuses_and_priorities(members):
                if st.failover_limitation() is None:
                    ...
                    if self.is_lagging(st.wal_position):
                        # This node appears to be lagging to us, doesn't matter if it's higher priority
                        continue
                    if self.get_effective_tags().get("failover_priority", 1) < priority:
                        # There's a higher-priority non-lagging replica
                        logger.info(
                            "Failover priority of %s is higher than mine",
                            st.member.name,
                        )
                        return False
                    if my_wal_position < st.wal_position:
                        logger.info(
                            "Wal position of %s is ahead of my wal position",
                            st.member.name,
                        )
                        ...
                        if (
                            not self.is_synchronous_mode()
                            or self.cluster.sync.is_empty
    ...
    def is_failover_possible(
        self,
        members: List[Member],
        check_synchronous: Optional[bool] = True,
        cluster_lsn: Optional[int] = 0,
    ) -> bool:
        ...
        if (
            check_synchronous
            and self.is_synchronous_mode()
            and not self.cluster.sync.is_empty
        ):
        ...
        if members:
            for st in self.fetch_nodes_statuses(members):
                ...
                if not_allowed_reason:
                    ...
                    cluster_lsn
                    and st.wal_position < cluster_lsn
                    or not cluster_lsn
                    and self.is_lagging(st.wal_position)
                ):
                    logger.info(
                        "Member %s exceeds maximum replication lag", st.member.name
                    )
                elif self.check_timeline() and (
                    not st.timeline or st.timeline < cluster_timeline
                ):
                    logger.info(
    ...
    def manual_failover_process_no_leader(self) -> Optional[bool]:
        ...
        if failover.candidate:  # manual failover to specific member
            if failover.candidate == self.state_handler.name:  # manual failover to me
                ...
                if (
                    not self.cluster.get_member(
                        failover.candidate, fallback_to_leader=False
                    )
            ...
            if self.is_synchronous_mode() and not self.cluster.sync.matches(
                self.state_handler.name, True
            ):
                return False

            # find specific node and check that it is healthy
            member = self.cluster.get_member(
                failover.candidate, fallback_to_leader=False
            )
        ...
        if failover.leader:
            if self.state_handler.name == failover.leader:  # I was the leader
                ...
                if self.is_failover_possible(
                    members
                ):  # check that there are healthy members
                    return False
    ...
    def is_healthiest_node(self) -> bool:
        ...
        if time.time() - self._released_leader_key_timestamp < self.dcs.ttl:
            logger.info(
                "backoff: skip leader race after pre_promote script failure and releasing the lock voluntarily"
            )
            return False
        ...
        if (
            ...
            if (
                ret is not None
            ):  # continue if we just deleted the stale failover key as a leader
                return ret
        ...
        if self.state_handler.is_leader():
            ...
            return (
                not self.is_paused()
                or not self.cluster.initialize
        ...
        if self.cluster.failover:
            ...
            if (
                self.is_synchronous_mode()
                and self.cluster.failover.leader
                and not self.cluster.sync.is_empty
        ...
        if self.is_synchronous_mode() and not self.cluster.sync.is_empty:
            ...
            members = {
                m.name: m
                for m in all_known_members
                if self.cluster.sync.matches(m.name, True)
            }
        else:
    ...
    def demote(self, mode: str) -> Optional[bool]:
        ...
        def on_shutdown(checkpoint_location: int) -> None:
            ...
            if self.is_failover_possible(
                self.cluster.members, cluster_lsn=checkpoint_location
            ):
        ...
        self.state_handler.stop(
            str(mode_control["stop"]),
            checkpoint=bool(mode_control["checkpoint"]),
        ...
        if mode_control["offline"]:
            ...
            try:
                cluster = self.dcs.get_cluster()
                node_to_follow, leader = (
                    self._get_node_to_follow(cluster),
                    cluster.leader,
                )
            except Exception:
                node_to_follow, leader = None, None
        ...
        if mode_control["async_req"]:
            self._async_executor.try_run_async(
                "starting after demotion", self.state_handler.follow, (node_to_follow,)
            )
        else:
    ...
    def should_run_scheduled_action(
        self,
        action_name: str,
        ...
        if scheduled_at and not self.is_paused():
            ...
            try:
                ...
                if delta > self.dcs.loop_wait:
                    ...
                    logger.warning(
                        "Found a stale %s value, cleaning up: %s",
                        action_name,
                        scheduled_at.isoformat(),
                    )
                    cleanup_fn()
                ...
                logger.info(
                    "Manual scheduled {0} at %s".format(action_name),
                    scheduled_at.isoformat(),
                )
                return True
            except TypeError:
    ...
    def process_manual_failover_from_leader(self) -> Optional[str]:
        ...
        if failover.scheduled_at and not self.should_run_scheduled_action(
            ...
            lambda: self.dcs.manual_failover("", "", version=failover.version),
        ):
            return
        ...
        if not failover.leader or failover.leader == self.state_handler.name:
            if not failover.candidate or failover.candidate != self.state_handler.name:
                if not failover.candidate and self.is_paused():
                    ...
                    if self.is_synchronous_mode():
                        if failover.candidate and not self.cluster.sync.matches(
                            failover.candidate
                        ):
                            logger.warning(
                                "Failover candidate=%s does not match with sync_standbys=%s",
                                failover.candidate,
                                self.cluster.sync.sync_standby,
                            )
                            members = []
                        else:
                            members = [
                    ...
                    if self.is_failover_possible(
                        ...
                        ret = self._async_executor.try_run_async(
                            "manual failover: demote", self.demote, ("graceful",)
                        )
    ...
    def process_unhealthy_cluster(self) -> str:
        ...
        if self.is_healthiest_node():
            ...
            if self.patroni.nofailover:
                return self.follow(
                    "demoting self because I am not allowed to become primary",
    ...
    def process_healthy_cluster(self) -> str:
        if self.has_lock():
            if self.is_paused() and not self.state_handler.is_leader():
                if (
                    ...
                    and self.cluster.failover.candidate == self.state_handler.name
                ):
                ...
                if not self.is_standby_cluster():
                    self._delete_leader()
                    return (
                        "removed leader lock because postgres is not running as primary"
                    )
            ...
            if self.update_lock(True):
                ...
                if self.is_standby_cluster():
                    ...
                    msg = (
                        "no action. I am ({0}), the standby leader with the lock".format(
                            self.state_handler.name
                        )
                        if self.state_handler.role == "standby_leader"
                        else "promoted self to a standby leader because i had the session lock"
                    )
                    return self.enforce_follow_remote_member(msg)
                else:
                    return self.enforce_primary_role(
                        "no action. I am ({0}), the leader with the lock".format(
                            self.state_handler.name
                        ),
        ...
        if self.is_standby_cluster():
            return self.follow(
                "cannot be a real primary in a standby cluster",
                "no action. I am ({0}), a secondary, and following a standby leader ({1})".format(
                    self.state_handler.name, lock_owner
                ),
                refresh=False,
            )
        return self.follow(
    ...
    def restart_matches(
        ...
        if postgres_version and postgres_version_to_int(postgres_version) <= int(
            self.state_handler.server_version
        ):
    ...
    def schedule_future_restart(self, restart_data: Dict[str, Any]) -> bool:
        with self._async_executor:
            ...
            ] = self.state_handler.postmaster_start_time()
    ...
    def restart(
        ...
        do_restart = functools.partial(
            self.state_handler.restart,
            timeout,
            self._async_executor.critical_task,
    ...
    def _do_reinitialize(self, cluster: Cluster) -> Optional[bool]:
        self.state_handler.stop(
            "immediate", stop_timeout=self.patroni.config["retry_timeout"]
        )
        ...
        if clone_member:
            ...
            return self.clone(
                clone_member, "from {0} '{1}'".format(member_role, clone_member.name)
            )
    ...
    def handle_long_action_in_progress(self) -> str:
        ...
        if self.has_lock() and self.update_lock():
            if (
                self._async_executor.scheduled_action
                == "doing crash recovery in a single user mode"
            ):
                time_left = self.global_config.primary_start_timeout - (
                    time.time() - self._crash_recovery_started
                )
                if time_left <= 0 and self.is_failover_possible(self.cluster.members):
                    logger.info(
                        "Demoting self because crash recovery is taking too long"
                    )
                    self.state_handler.cancellable.cancel(True)
                    self.demote("immediate")
                    return "terminated crash recovery because of startup timeout"

            return "updated leader lock during {0}".format(
                self._async_executor.scheduled_action
            )
            ...
            if self.state_handler.role in ("master", "primary"):
                logger.info(
                    "Demoting primary during %s", self._async_executor.scheduled_action
                )
                if self._async_executor.scheduled_action in (
                    "restart",
                    "starting primary after failure",
                ):
                    ...
                    with self._async_executor.critical_task as task:
                        if not task.cancel() and isinstance(
                            task.result, PostmasterProcess
                        ):
                            self.state_handler.terminate_starting_postmaster(
                                postmaster=task.result
                            )
                self.demote("immediate-nolock")
                return "lost leader lock during {0}".format(
                    self._async_executor.scheduled_action
                )
        if self.cluster.is_unlocked():
    ...
    def post_bootstrap(self) -> str:
        ...
        if result is None:
            ...
            ret = self._async_executor.try_run_async(
                "post_bootstrap",
                self.state_handler.bootstrap.post_bootstrap,
        ...
        self._rewind.ensure_checkpoint_after_promote(self.wakeup)
        self.dcs.initialize(
            create_new=(self.cluster.initialize is None), sysid=self.state_handler.sysid
        )
        self.dcs.set_config_value(
            json.dumps(self.patroni.config.dynamic_configuration, separators=(",", ":"))
        )
    ...
    def handle_starting_instance(self) -> Optional[str]:
        ...
        if not self.state_handler.check_for_startup() or self.is_paused():
            ...
            if self.is_paused():
                self.state_handler.set_state(
                    self.state_handler.is_running() and "running" or "stopped"
                )
            return None
        ...
        if self.has_lock():
            ...
            if time_left <= 0:
                if self.is_failover_possible(self.cluster.members):
                    logger.info(
                        "Demoting self because primary startup is taking too long"
                    )
                    self.demote("immediate")
                ...
                return "PostgreSQL is still starting up, {0:.0f} seconds until timeout".format(
                    time_left
                )
        else:
        ...
        try:
            try:
                ...
                self.state_handler.reset_cluster_info_state(
                    self.cluster, self.patroni.nofailover, self.global_config
                )
            except Exception:
            ...
            if (
                not (
                    self.cluster.is_unlocked()
                    or self.sysid_valid(self.cluster.initialize)
                )
                and self.has_lock()
            ):
                self.dcs.initialize(
                    create_new=(self.cluster.initialize is None),
                    sysid=self.state_handler.sysid,
                )

            if (
                not (
                    self.cluster.is_unlocked()
                    or self.cluster.config
                    and self.cluster.config.data
                )
                and self.has_lock()
            ):
                self.dcs.set_config_value(
                    json.dumps(
                        self.patroni.config.dynamic_configuration, separators=(",", ":")
                    )
                )
            ...
            if not data_directory_is_accessible or data_directory_is_empty:
                ...
                if not data_directory_is_accessible:
                    return "data directory is not accessible: {0}".format(
                        data_directory_error
                    )
                if self.is_paused():
                ...
                if self.sysid_valid(self.cluster.initialize):
                    if self.cluster.initialize != data_sysid:
                        if self.is_paused():
                            logger.warning(
                                "system ID has changed while in paused mode. Patroni will exit when resuming"
                                " unless system ID is reset: %s != %s",
                    ...
                    if (
                        not self.state_handler.cb_called
                        and self.state_handler.is_running()
                        ...
                        logger.error(
                            "Please first start Patroni on the node running as primary"
                        )
                        sys.exit(1)
                    self.dcs.initialize(
                        create_new=(self.cluster.initialize is None), sysid=data_sysid
                    )
            ...
            if (
                not self._async_executor.busy or is_promoting
                ...
                if not is_promoting and create_slots and self.cluster.leader:
                    err = self._async_executor.try_run_async(
                        "copy_logical_slots",
    ...
    def _handle_dcs_error(self) -> str:
        if not self.is_paused() and self.state_handler.is_running():
            if self.state_handler.is_leader():
                ...
                if not self._async_executor.try_run_async(
                    msg, self.demote, ("offline",)
                ):
                    return msg
    ...
    def _sync_replication_slots(self, dcs_failed: bool) -> List[str]:
        ...
        if (
            not self.cluster
            or dcs_failed
            and (self.is_leader() or not self.is_failsafe_mode())
        ):
            return slots
        ...
        cluster = (
            self._failsafe.update_cluster(self.cluster)
            if self.is_failsafe_mode() and not self.is_leader()
            else self.cluster
        )
        if cluster:
            slots = self.state_handler.slots_handler.sync_replication_slots(
                cluster,
    ...
    def shutdown(self) -> None:
        if self.is_paused():
            ...
            self.while_not_sync_standby(
                lambda: self.state_handler.stop(
                    checkpoint=False,
                    on_safepoint=disable_wd,
            ...
            if not self.state_handler.is_running():
                ...
                logger.error(
                    "PostgreSQL shutdown failed, leader key not removed.%s",
    ...
    def watch(self, timeout: float) -> bool:
        ...
        if (
            self._async_executor.busy
            or not self.cluster
            or self.cluster.is_unlocked()
            or self.has_lock(False)
        ):
            leader_version = None
        else:
            leader_version = (
                self.cluster.leader.version if self.cluster.leader else None
            )
    ...
    def get_remote_member(
        self, member: Union[Leader, Member, None] = None
    ) -> RemoteMember:
