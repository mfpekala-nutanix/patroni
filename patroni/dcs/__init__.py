...
def dcs_modules() -> List[str]:
    ...
    if getattr(sys, "frozen", False):
        ...
        return [
            module
            for module in toc
            if module.startswith(module_prefix) and module.count(".") == 2
        ]
    else:
        return [
            module_prefix + name
            for _, name, is_pkg in pkgutil.iter_modules([dcs_dirname])
            if not is_pkg
        ]
...
def get_dcs(config: Union["Config", Dict[str, Any]]) -> "AbstractDCS":
    ...
    for module_name in modules:
        ...
        if (
            name in config
        ):  # we will try to import only modules which have configuration section in the config file
            try:
                module = importlib.import_module(module_name)
                for (
                    key,
                    item,
                ) in module.__dict__.items():  # iterate through the module content
                    # try to find implementation of AbstractDCS interface, class name must match with module_name
                    if (
                        key.lower() == name
                        and inspect.isclass(item)
                        and issubclass(item, AbstractDCS)
                    ):
    ...
    for module_name in modules:
        ...
        try:
            module = importlib.import_module(module_name)
            available_implementations.extend(
                name
...
Available implementations: """
        + ", ".join(sorted(set(available_implementations)))
    )
...
class Member(NamedTuple):
    ...
    @staticmethod
    def from_node(
        version: _Version, name: str, session: _Session, value: str
    ) -> "Member":
        """
    ...
    def conn_url(self) -> Optional[str]:
        ...
        if conn_kwargs:
            conn_url = uri(
                "postgresql", (conn_kwargs.get("host"), conn_kwargs.get("port", 5432))
            )
            self.data["conn_url"] = conn_url
            return conn_url

    def conn_kwargs(
        self, auth: Union[Any, Dict[str, Any], None] = None
    ) -> Dict[str, Any]:
        defaults = {"host": None, "port": None, "dbname": None}
        ...
        if ret:
            ...
            if not conn_url:
                return (
                    {}
                )  # due to the invalid conn_url we don't care about authentication parameters
            r = urlparse(conn_url)
            ret = {"host": r.hostname, "port": r.port or 5432, "dbname": r.path[1:]}
...
class Leader(NamedTuple):
    ...
    def checkpoint_after_promote(self) -> Optional[bool]:
        ...
        if version and version > (1, 5, 6):
            return (
                self.data.get("role") in ("master", "primary")
...
class Failover(NamedTuple):
    ...
    def from_node(version: _Version, value: Union[str, Dict[str, str]]) -> "Failover":
        ...
        return Failover(
            version, data.get("leader"), data.get("member"), data.get("scheduled_at")
        )
...
class ClusterConfig(NamedTuple):
    ...
    @staticmethod
    def from_node(
        version: _Version, value: str, modify_version: Optional[_Version] = None
    ) -> "ClusterConfig":
        """
        ...
        return ClusterConfig(
            version, data, version if modify_version is None else modify_version
        )
...
class SyncState(NamedTuple):
    ...
    @staticmethod
    def from_node(
        version: Optional[_Version], value: Union[str, Dict[str, Any], None]
    ) -> "SyncState":
        """
    ...
    def members(self) -> List[str]:
        ...
        return (
            self._str_to_list(self.sync_standby)
            if not self.is_empty and self.sync_standby
            else []
        )
    ...
    def matches(self, name: Optional[str], check_leader: bool = False) -> bool:
        ...
        if name and not self.is_empty:
            search_str = (self.sync_standby or "") + (
                "," + (self.leader or "") if check_leader else ""
            )
    ...
    def leader_matches(self, name: Optional[str]) -> bool:
        ...
        return bool(
            name and not self.is_empty and name.lower() == (self.leader or "").lower()
        )


_HistoryTuple = Union[
    Tuple[int, int, str], Tuple[int, int, str, str], Tuple[int, int, str, str, str]
]
...
class Cluster(NamedTuple):
    ...
    def empty() -> "Cluster":
        return Cluster(
            None, None, None, 0, [], None, SyncState.empty(), None, None, None
        )
    ...
    def is_empty(self):
        return (
            ...
            and self.sync.version is None
            and self.history is None
            and self.slots is None
            and self.failsafe is None
            and self.workers == {}
        )
    ...
    def get_member(
        self, member_name: str, fallback_to_leader: bool = True
    ) -> Union[Member, Leader, None]:
        return (
            [m for m in self.members if m.name == member_name]
            or [self.leader if fallback_to_leader else None]
        )[0]
    ...
    def get_clone_member(self, exclude_name: str) -> Union[Member, Leader, None]:
        ...
        candidates = [
            ...
            if m.clonefrom and m.is_running and m.name not in exclude
        ]
        return (
            candidates[randint(0, len(candidates) - 1)] if candidates else self.leader
        )
    ...
    def __permanent_physical_slots(self) -> Dict[str, Any]:
        return {
            name: value
            for name, value in self.__permanent_slots.items()
            if not value
    ...
    def use_slots(self) -> bool:
        return bool(
            self.config
            and (self.config.data.get("postgresql") or {}).get("use_slots", True)
        )
    ...
    def get_replication_slots(
        ...
        if role in ("master", "primary", "standby_leader"):
            slot_members = [
                ...
                and m.name != my_name
                and (
                    m.replicatefrom is None
                    or m.replicatefrom == my_name
                    or not self.has_member(m.replicatefrom)
                )
            ]
            permanent_slots = (
                self.__permanent_slots
                if use_slots and role in ("master", "primary")
            ...
            slot_members = [
                ...
                for m in self.members
                if use_slots
                and m.replicatefrom == my_name
                and m.name != self.leader_name
            ]
            permanent_slots = (
                self.__permanent_logical_slots if use_slots and not nofailover else {}
            )

        slots = {
            slot_name_from_member_name(name): {"type": "physical"}
        ...
        if len(slots) < len(slot_members):
            ...
            logger.error(
                "Following cluster members share a replication slot name: %s",
                "; ".join(
                    "{} map to {}".format(", ".join(v), k)
                    for k, v in slot_conflicts.items()
                    if len(v) > 1
                ),
            )
        ...
        for name, value in permanent_slots.items():
            if not slot_name_re.match(name):
                ...
                logger.error(
                    "Slot name may only contain lower case letters, numbers, and the underscore chars"
                )
                continue
            ...
            if isinstance(value, dict):
                if "type" not in value:
                    value["type"] = (
                        "logical"
                        if value.get("database") and value.get("plugin")
                        else "physical"
                    )
                ...
                if value["type"] == "physical":
                    ...
                    if major_version < 110000:
                        ...
                        logger.error(
                            "Permanent logical replication slot {'%s': %s} is conflicting with"
                            " physical replication slot for cluster member",
                            name,
                            value,
                        )
                    else:
            ...
            logger.error(
                "Bad value for slot '%s' in permanent_slots: %s",
                name,
                permanent_slots[name],
            )

        if disabled_permanent_logical_slots and show_error:
            logger.error(
                "Permanent logical replication slots supported by Patroni only starting from PostgreSQL 11. "
                "Following slots will not be created: %s.",
                disabled_permanent_logical_slots,
            )

        return slots

    def has_permanent_logical_slots(
        self, my_name: str, nofailover: bool, major_version: int = 110000
    ) -> bool:
        if major_version < 110000:
            return False
        slots = self.get_replication_slots(
            my_name, "replica", nofailover, major_version
        ).values()
        return any(v for v in slots if v.get("type") == "logical")

    def should_enforce_hot_standby_feedback(
        self, my_name: str, nofailover: bool, major_version: int
    ) -> bool:
        """
        ...
        if self.use_slots:
            ...
            return any(
                self.should_enforce_hot_standby_feedback(
                    m.name, m.nofailover, major_version
                )
                for m in members
            )
        return False

    def get_my_slot_name_on_primary(
        self, my_name: str, replicatefrom: Optional[str]
    ) -> str:
        """
        P <-- I <-- L
        ...
        return (
            self.get_my_slot_name_on_primary(m.name, m.replicatefrom)
            if isinstance(m, Member)
            else slot_name_from_member_name(my_name)
        )
        ...
        if self.history:
            if self.history.lines:
                try:
                    ...
                    logger.error(
                        "Failed to parse cluster history from DCS: %s",
                        self.history.lines,
                    )
    ...
    def min_version(self) -> Optional[Tuple[int, ...]]:
        return next(
            iter(sorted(m.patroni_version for m in self.members if m.patroni_version)),
            None,
        )
...
class AbstractDCS(abc.ABC):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self._base_path = re.sub(
            "/+",
            "/",
            "/".join(["", config.get("namespace", "service"), config["scope"]]),
        )
        self._citus_group = (
            str(config["group"]) if isinstance(config.get("group"), int) else None
        )
    ...
    def get_citus_coordinator(self) -> Optional[Cluster]:
        try:
            return self.__get_patroni_cluster(
                "{0}/{1}/".format(self._base_path, CITUS_COORDINATOR_GROUP_ID)
            )
        except Exception as e:
            logger.error(
                "Failed to load Citus coordinator cluster from %s: %r",
    ...
    def get_cluster(self, force: bool = False) -> Cluster:
        ...
        try:
            cluster = (
                self._get_citus_cluster()
                if self.is_citus_coordinator()
                else self.__get_patroni_cluster()
            )
        except Exception:
            self.reset_cluster()
            raise
    ...
    def write_status(self, value: Dict[str, Any]) -> None:
        if not deep_compare(self._last_status, value) and self._write_status(
        ...
        if (
            min_version
            and min_version < (2, 1, 0)
            and self._last_lsn != value[self._OPTIME]
        ):
    ...
    def write_failsafe(self, value: Dict[str, str]) -> None:
        if not (
            isinstance(self._last_failsafe, dict)
            and deep_compare(self._last_failsafe, value)
    ...
    def manual_failover(
        ...
        return self.set_failover_value(
            json.dumps(failover_value, separators=(",", ":")), version
        )
    ...
    @staticmethod
    def sync_state(
        leader: Optional[str], sync_standby: Optional[Collection[str]]
    ) -> Dict[str, Any]:
    ...
    def write_sync_state(
        self,
        leader: Optional[str],
        sync_standby: Optional[Collection[str]],
        version: Optional[Any] = None,
    ) -> Optional[SyncState]:
        ...
        ret = self.set_sync_state_value(
            json.dumps(sync_value, separators=(",", ":")), version
        )
    ...
    @abc.abstractmethod
    def set_sync_state_value(
        self, value: str, version: Optional[Any] = None
    ) -> Union[Any, bool]:
