...
class GlobalConfig(object):
    ...
    def is_standby_cluster(self) -> bool:
        ...
        return isinstance(config, dict) and bool(
            config.get("host") or config.get("port") or config.get("restore_command")
        )
    ...
    def synchronous_node_count(self) -> int:
        ...
        return max(
            self.get_int("synchronous_node_count", 1), self.min_synchronous_nodes
        )
    ...
    def primary_start_timeout(self) -> int:
        ...
        default = 300
        return (
            self.get_int("primary_start_timeout", default)
            if "primary_start_timeout" in self.__config
            else self.get_int("master_start_timeout", default)
        )
    ...
    def primary_stop_timeout(self) -> int:
        ...
        default = 0
        return (
            self.get_int("primary_stop_timeout", default)
            if "primary_stop_timeout" in self.__config
            else self.get_int("master_stop_timeout", default)
        )


def get_global_config(
    cluster: Union[Cluster, None], default: Optional[Dict[str, Any]] = None
) -> GlobalConfig:
...
class Config(object):
    ...
    __DEFAULT_CONFIG: Dict[str, Any] = {
        ...
        "postgresql": {
            ...
            "parameters": CaseInsensitiveDict(
                {
                    p: v[0]
                    for p, v in ConfigHandler.CMDLINE_OPTIONS.items()
    ...
    def __init__(
        self,
        configfile: str,
        validator: Optional[Callable[[Dict[str, Any]], List[str]]] = default_validator,
    ) -> None:
        ...
        self._config_file = (
            configfile if configfile and os.path.exists(configfile) else None
        )
        if self._config_file:
            ...
            self._local_configuration = (
                config_env
                and yaml.safe_load(config_env)
                or self.__environment_configuration
            )
        ...
        self._data_dir = self.__effective_configuration.get("postgresql", {}).get(
            "data_dir", ""
        )
        ...
        if self._cache_needs_saving:
            ...
            try:
                (fd, tmpfile) = tempfile.mkstemp(
                    prefix=self.__CACHE_FILENAME, dir=self._data_dir
                )
    ...
    def set_dynamic_configuration(
        self, configuration: Union[ClusterConfig, Dict[str, Any]]
    ) -> bool:
        ...
        if not deep_compare(self._dynamic_configuration, configuration):
            try:
                self.__effective_configuration = self._build_effective_configuration(
                    configuration, self._local_configuration
                )
        ...
        if self.config_file:
            try:
                ...
                if not deep_compare(self._local_configuration, configuration):
                    new_configuration = self._build_effective_configuration(
                        self._dynamic_configuration, configuration
                    )
                ...
                logger.exception(
                    "Exception when reloading local configuration from %s",
                    self.config_file,
                )

    @staticmethod
    def _process_postgresql_parameters(
        parameters: Dict[str, Any], is_local: bool = False
    ) -> Dict[str, Any]:
        return {
            name: value
            ...
            or not is_local
            and ConfigHandler.CMDLINE_OPTIONS[name][1](value)
        }

    def _safe_copy_dynamic_configuration(
        self, dynamic_configuration: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for name, value in dynamic_configuration.items():
            if name == "postgresql":
                for name, value in (value or {}).items():
                    if name == "parameters":
                        config["postgresql"][name].update(
                            self._process_postgresql_parameters(value)
                        )
                    elif name not in (
                        "connect_address",
                        "proxy_address",
                ...
                name in config
            ):  # only variables present in __DEFAULT_CONFIG allowed to be overridden from DCS
    ...
    def _build_environment_configuration() -> Dict[str, Any]:
        ...
        _set_section_values(
            "ctl", ["insecure", "cacert", "certfile", "keyfile", "keyfile_password"]
        )
        _set_section_values(
            "postgresql",
            [
        ...
        _set_section_values(
            "raft", ["data_dir", "self_addr", "partner_addrs", "password", "bind_addr"]
        )
        ...
        def _get_auth(
            name: str, params: Optional[Collection[str]] = None
        ) -> Dict[str, str]:
            ret: Dict[str, str] = {}
    ...
    def _build_effective_configuration(
        ...
        local_configuration: Dict[str, Union[Dict[str, Any], Any]],
    ) -> Dict[str, Any]:
        ...
        for name, value in local_configuration.items():
            if name == "citus":  # remove invalid citus configuration
                if (
                    ...
                    and isinstance(value.get("group"), int)
                ...
                for name, value in (value or {}).items():
                    if name == "parameters":
                        config["postgresql"][name].update(
                            self._process_postgresql_parameters(value, True)
                        )
                    elif (
                        name != "use_slots"
                    ):  # replication slots must be enabled/disabled globally
                        config["postgresql"][name] = deepcopy(value)
        ...
        if "restapi" in config and "authentication" in config["restapi"]:
            config["restapi"]["auth"] = "{username}:{password}".format(
        ...
        if "authentication" not in pg_config:
            ...
            pg_config["authentication"] = {
                u: pg_config[u] for u in ("replication", "superuser") if u in pg_config
            }
        ...
        for name, value in pg_config["authentication"].items():
            pg_config["authentication"][name] = {
                n: v for n, v in value.items() if n in _AUTH_ALLOWED_PARAMETERS
            }
        ...
        updated_fields = ("name", "scope", "retry_timeout", "citus")
    ...
    def _validate_failover_tags(self):
        """Ensures that only one of `nofailover` and `failover_priority` is set"""
        has_no_failover_tag = self.get("tags", {}).get("nofailover", None) is not None
        has_failover_priority_tag = (
            self.get("tags", {}).get("failover_priority", None) is not None
        )
        if has_no_failover_tag and has_failover_priority_tag:
            raise ConfigParseError(
                "Cannot provide values for both `nofailover` and `failover_priority`. Please set only one and try again."
            )
